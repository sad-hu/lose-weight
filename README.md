...
> 这是我在我们公司 FT（前端组）组里写的工具，主要针对前端在收到接口返回数据后的字段处理，在我们公司若干个实际项目中，接口返回的数据，有些字段是可以在视图层直接展示或修改，有些字段需要事先进行格式化，有些字段是完全无需的。本工具将有用字段的提取（解构）代码和字段的格式化代码集中在一起处理！
> 
> 希望这个工具也能给大家在实际工作中带来便捷！
...
2. 第 2 个可选，默认值 undefined（示例中的 options）是当前字段的选项
    ...
    例如 source 某项中某个字段名为 level，且传递了通过链式调用 opt 传递了 level 这个字段的选项，那么在相应字段处理函数中第 2 个参数即为 `['新手', '普通', '专家']`
...
注意：任何与贡献要求有冲突的地方，可以发 issue 讨论，另外我们 FT 组内也会对此进行讨论和决议！
